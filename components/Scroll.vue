...
<script>
  ...
  export default {
    props: ['type','top','right','bottom','size','color','duration','el_class'],
    data () {
      return {
        showIcon: false,
        scrollTop: 0,
    ...
    mounted () {
      ...
      this.el_class ? this.defaultElClass = this.el_class: this.defaultElClass ;
      window.addEventListener('scroll', this.handelScroll, true)
    },
    methods: {
      handelScroll () {
        let element= this.defaultElClass?document.getElementsByClassName(this.defaultElClass)[0]:document.documentElement
